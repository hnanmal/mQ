...
from src.views.widget.calc_sheet_widget import (
    ReportGroup_SheetWidget,
    ReportMember_SheetWidget,
)


def create_report_group_tab(state, subtab_notebook, exe_mode=None):
    ...
    if not exe_mode:
        subtab_notebook.add(working_tab, text="Q'ty Report by Group")
    ...
    report_group_calc_sheet = ReportGroup_SheetWidget(
        state, section1, data_kind="dynamo-calculation"
    )
    report_group_calc_sheet.pack(fill=tk.BOTH, expand=True)

    # ######## notify_targets 등록 ###############################################
    # state.notify_targets.append(report_group_calc_sheet)
    # ############################################################################
