...
from src.views.widget.calc_sheet_widget import (
    ReportBuildingTotal_SheetWidget,
    ReportMember_SheetWidget,
)


def create_report_TotalBD_tab(state, subtab_notebook, exe_mode=None):
    ...
    if not exe_mode:
        subtab_notebook.add(working_tab, text="Q'ty Report to Total BOQ")
    ...
    report_sheet = ReportBuildingTotal_SheetWidget(
        state, section1, data_kind="dynamo-calculation"
    )
    report_sheet.pack(fill=tk.BOTH, expand=True)
