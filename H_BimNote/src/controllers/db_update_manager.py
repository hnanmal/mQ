...
class DBUpdateManager:
    ...
    def handle_stdGWM_treeview_selection(self, tree, tree_item_id):
        # Fetch the data to be displayed in the listbox based on the selected tree item
        listbox_data = self.state.get_stdGWM_matching(tree_item_id)
        # Update the state
        self.state.update_stdGWM_matching(tree_item_id, listbox_data)

    def handle_stdGWM_listbox_edit(self, tree_item_id, updated_listbox_items):
        # Update the state when the listbox is edited
        self.state.update_stdGWM_matching(tree_item_id, updated_listbox_items)
