...
;; 이 정도면 최종 리턴한 결과를 reverse 한다는 점이 거슬린다는 점 외에는 나쁘지 않은 코드다. 원하는 값을 생성하기 위해 전체 리스트를 탐색하고 나면 결과 값의 순서를 바로잡기 위해 reverse가 사용된다. 비효율적이면서도 불필요한 동작이다. reverse를 사용하지 않을 방법은 누산기에 리스트 대신 벡터를 사용하는 것이다.
...
;; 5.2.5 서브 벡터
;; 벡터는 오른쪽 끝에 있는 항목 외에는 삭제 동작이 효율적이지 않지만, a-to-j (앞에서 [\A \B \C \D \E \F \G \H \I \J] 로 정의했던) 와 같은 기존의 벡터의 시작과 끝을 지정하여 일부 조각을 꺼낼 수 있는 빠른 방법으로 subvec이라는 함수를 제공하고 있다.
(subvec a-to-j 3 6)

;; subvec에 주어진 인덱스에서 첫 번째 인덱스는 (3’에서’ 시작) 포함되고 두 번째 인덱스는 포함되지 않는다(6 ‘전에서’ 끝). 새 서브 벡터는 내부적으로는 원래의 벡터 a-to-j에 그대로 붙어있는데, 각 조회는 새 벡터에서 수행되기 때문에 서브 벡터에서 오프셋을 계산하고 이를 원래의 벡터에서 가져온다. 이와 같은 원리로 서브 벡터가 빠르게 생성된다. 어떤 종류의 벡터에라도 subvec을 사용할 수 있고, 잘 동작한다. 그러나 서브 벡터의 서브 벡터를 생성할 때는 중간에 생성된 벡터들이 아닌 가장 최신의 서브 벡터에만 원래 벡터의 참조 정보를 유지하는 특별한 로직이 적용된다. 이러한 방식은 서브 벡터의 서브 벡터가 불필요하게 쌓이는 것을 방지하고, 이를 빠르고 효율적으로 생성하고 사용할 수 있도록 해준다.

;; 5.2.6 맵 엔트리로서의 벡터
;; 다른 많은 언어들의 해시 테이블 사전처럼 클로저의 해시 맵 또한 전체 컬렉션을 반복하는 메커니즘을 갖고 있다. 예상했다시피, 클로저에서 이 반복 메커니즘은 시퀀스 (seq) 에 의해 이루어진다. 시퀀스의 각 아이템은 키와 값을 모두 포함해야 하므로 맵 엔트리로 구성되어 있다. 출력된 것을 보면 각 엔트리들이 마치 벡터처럼 보인다.

(first {:width 10 :height 20 :depth 15})

(vector? (first {:width 10 :height 20 :depth 15}))

;; 이 말은 여기에 conj, get 등의 벡터 함수들을 적용할 수 있다는 말이다. 심지어 구조분해까지 지원해서 편리하게 사용할 수 있다. 예를 들어, 다음 코드의 로컬 dimension과 amount는 각각 키와 값 쌍을 맡는다.
(doseq [[dimension amount] {:width 10 :height 20 :depth 15}]
  (println (str (name dimension) ":") amount "inches"))

;; MapEntry는 key와 val을 조회하는 두 개의 함수를 갖는 독립적인 타입이다. key와 val은 각각 (nth my-map 0) 와 (nth my-map 1) 에 해당한다. MapEntry는 코드를 명확하게 만들어준다는 점에서 유용하기는 하지만, 구조분해가 그 편리성 때문에 더 사용되는 편이다.
;; 지금까지 벡터가 무엇인지, 클로저에 어떤 종류들이 있는지, 또 벡터가 유용하게 사용되는지에 대해 알아보았다. 이제 좀 더 이해를 넓혀보기 위해 벡터를 사용하기에 적절하지 않은 경우에 대해서도 간단하게 살펴보자.