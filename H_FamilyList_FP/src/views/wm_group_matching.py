...
from src.views.listbox_utils import add_selected_row_to_listbox, display_level_6_items_list, remove_selected_items_from_listbox
...
def create_wm_group_matching_tab(notebook, state, wm_group_manager):
    ...
    def on_select_item(item_name):
        section2_label.config(text=f"{item_name}")
    ...
    section2_label = ttk.Label(section2, text="Selected Item: None", font=("Arial", 14))
    section2_label.grid(row=0, column=0, columnspan=2, pady=20)

    # Create a frame to hold the Listbox and scrollbars
    center_listbox_frame = ttk.Frame(section2)
    center_listbox_frame.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")
    ...
    center_listbox = tk.Listbox(center_listbox_frame, selectmode=tk.EXTENDED,
                                yscrollcommand=vertical_scrollbar.set,
                                xscrollcommand=horizontal_scrollbar.set,
                                height=15, font=("Arial", 10), activestyle="dotbox",
                                )
    ...
    add_button = ttk.Button(button_frame, text="Add", command=lambda: add_selected_row_to_listbox(sheet_widget, center_listbox))
    add_button.pack(side=tk.TOP, pady=5)

    # Del button to remove selected items from center listbox
    del_button = ttk.Button(button_frame, text="Del", command=lambda: remove_selected_items_from_listbox(center_listbox))
    del_button.pack(side=tk.TOP, pady=5)

    # Lock button to lock the listbox and disable add/del buttons
    lock_button = ttk.Button(section2, text="Lock", command=lambda: lock_toggle_logic(state, wm_group_manager, section2_label['text'], center_listbox, add_button, del_button))
    lock_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew")

    # Display the level 6 items in Section 1 as a Listbox
    display_level_6_items_list(section1, on_select_item)

    # In the third section, read and display the Excel file using tksheet
    sheet_widget = load_and_display_excel_with_search(section3)
