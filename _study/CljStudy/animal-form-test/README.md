...
* [`src/animal_form_test/`](src/animal_form_test/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/animal_form_test/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `animal-form-test` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/animal_form_test/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns animal-form-test.example
  (:require [animal-form-test.config :as config])
