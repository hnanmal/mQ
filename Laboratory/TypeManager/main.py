...
class App(tk.Tk):
    def __init__(self):
        ...
        for i, name in enumerate(tab_names):
            if name == "형식 표준 구성도":
                ...
                self.create_three_area_tab(i + 1, name)
    ...
    def create_single_area_tab(self, name):
        ...
        # Create a Treeview widget for the tree structure
        self.tree = ttk.Treeview(tab_frame)
        ...
        self.tree.column("original_name", width=0, minwidth=0, stretch=False)
        self.tree.column("display_name", width=300, minwidth=200)
        self.tree.column("description", width=400, minwidth=200)
        self.tree.column("indent", width=0, minwidth=0, stretch=False)

        # Define headings
        self.tree.heading("#0", text="#")
        self.tree.heading("display_name", text="Item")
        self.tree.heading("description", text="Description")

        # Add initial top-level items to the tree
        for i, item in enumerate(self.top_level_items):
            self.add_numbered_item("", f"{i}", item, f"{item} description", 0)

        # Pack the Treeview widget below the buttons
    ...
    def create_three_area_tab(self, n, name):
        # Create a frame for the tab
        tab_frame = ttk.Frame(self.notebook)
    ...
    def add_numbered_item(self, parent, number, original_name, description, indent):
        item_id = self.tree.insert(
            parent, "end", text=number, values=(original_name, "", description, indent)
        )
        self.update_displayed_name(item_id)
        return item_id

    def update_displayed_name(self, item):
        # Retrieve the item's indent level and original name
        indent_level = self.tree.set(item, "indent")
        original_name = self.tree.set(item, "original_name")
        # Indent the display name with spaces based on the indent level
        indented_name = " " * (int(indent_level) * 4) + original_name
        # Update the displayed name
        self.tree.set(item, "display_name", indented_name)
    ...
    def add_item(self):
        ...
        if not selected_item:
            ...
            new_item_id = self.add_numbered_item(
                "", str(new_index), "New Top-level Item", "New top-level description", 0
            )
            self.tree.selection_set(new_item_id)
        else:
            # Add a sub-item under the selected item
            parent_indent = int(self.tree.set(selected_item, "indent"))
    ...
    def on_item_double_click(self, event):
        ...
        if column == "#1":  # Display name column
            original_name = self.tree.set(
                selected_item, "original_name"
            )  # Get original name
            ...
            entry.bind(
                "<Return>", lambda e: self.on_item_edit(entry, selected_item, column)
            )
            entry.bind(
                "<FocusOut>", lambda e: self.on_item_edit(entry, selected_item, column)
            )
    ...
    def get_item_data(self, item):
        item_data = {
            ...
            "name": self.tree.set(
                item, "original_name"
            ),  # Save the original name without indentation
            "description": self.tree.set(item, "description"),
            "indent": self.tree.set(item, "indent"),
            "children": [
